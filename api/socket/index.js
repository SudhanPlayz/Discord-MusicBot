            ...
            socket.Server = setInterval(async () => {
                ...
                if(!player){
                    ...
                    socket.emit("server", {
                        ...
                        prefix: GuildDB?GuildDB.prefix:Client.config.DefaultPrefix,
                        bar: player.queue.current?Client.ProgressBar(player.position, player.queue.current.duration, 20).Bar:false,
                        maxDuration: player.queue.current?prettyMilliseconds(player.queue.current.duration, {colonNotation: true}):false,
                        position: player.queue.current?prettyMilliseconds(player.position, {colonNotation: true}):false,
