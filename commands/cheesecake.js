...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Usage - **\`${GuildDB.prefix}cheesecake [playlist message link]\``
      );
    let ref_message;
    const link = message.content.split("/");
    let guild = client.guilds.cache.get(link[4]);
    let channel = guild.channels.cache.get(link[5]);
    ref_message = await channel.messages.fetch(link[6]);
    // .then(ref_message => )
    // .catch(console.error);
    // let ref_message = message.channel.messages.fetch(link[6]);
    let song_list = ref_message.content.split(/\r?\n/).slice(2, -1);
    message.channel.send(
      "Cheesecake is being enjoyed with playlist -> " +
        ref_message.content.split(/\r?\n/)[0]
    );
