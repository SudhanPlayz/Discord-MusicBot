...
module.exports = {
  ...
  run: async function (client, message, args) {
   const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError("There are no songs in the queue.",message.channel).catch(console.error);
    ...
    if (queue.songs.length == 1) return sendError("There are no songs in the queue.",message.channel).catch(console.error);
