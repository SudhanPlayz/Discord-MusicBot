...
const e621404 = "https://img.lovepik.com/element/40021/7866.png_1200.png";
...
const command = new SlashCommand()
  .setName("e621")
  .setDescription("Search on e621 for a random image")
  .addStringOption(
  ...
  .setRun(async (client, interaction) => {
    ...
    // const BASE_URL = `https://e621.net/posts.json?tags=${search}+order:random+favcount:>${limitFavCount}+-filetype:swf&limit=1`;

    const BASE_URL = `https://e621.net/posts.json?tags=${search}+order:random+favcount:>${limitFavCount}&limit=1`;
    ...
    try {
      ...
      console.log("prev_link: " + getValue(data.preview, "url") || e621404);
      ...
      const e621EmbedIntro = new MessageEmbed()
        .setTitle(
          `[${
            interaction.member.nickname || interaction.user.username
          }] wants to find a <${search || "RANDOM"}> image on e621 *notices your bulge* UwU What's This?`
        )
        .setAuthor({ name: "e621", iconURL: e621ICON, proxyIconURL: e621ICON })
      ...
      const e621EmbedMeta = new MessageEmbed()
        .setThumbnail(getValue(data.preview, "url") != "N/A" ? getValue(data.preview, "url") : e621404)
        .setFields([
          {
            name: "Meta",
            value: `\`\`\`yml\nID: ${getValue(
      ...
      const e621EmbedTags = new MessageEmbed()
      ...
      const e621EmbedSource = new MessageEmbed().setFields([
        {
          name: `Sources`,
      ...
      const imgURL = getValue(data.file, "url") === "N/A" ?  e621404 : getValue(data.file, "url")

      const e621Attach = new MessageAttachment()
        .setFile(imgURL)
        .setName(
