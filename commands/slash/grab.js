...
const command = new SlashCommand()
  ...
  .setRun(async (client, interaction) => {
    ...
    if (!interaction.member.voice.channel) {
      ...
      .setColor(client.config.embedColor)
      .setDescription(":x: | **You must be in a voice channel to use this command!**");
    ...
    const save = new MessageEmbed()
        .setColor(client.config.embedColor)
        .setAuthor({ name: "Saved track", iconURL: `${interaction.user.displayAvatarURL({ dynamic: true })}`})
    ...
    return interaction.reply({ embeds: [new MessageEmbed()
        .setColor(client.config.embedColor)
        .setDescription("Please check your **DM**. If you don't receive any message from me please make sure your **DM** is open")
    ], ephemeral: true});
  });
