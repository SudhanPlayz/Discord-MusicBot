  ...
  rl.question(
    ...
    async (guild) => {
      ...
      for (let i = 0; i < commands.length; i++) {
          ...
          await rest.delete(Routes.applicationGuildCommand(config.clientId, guild, cmd.id))
          console.log("Deleted command: "+cmd.name)
      }
      if(commands.length === 0)console.log("Evil bot doesn't seen any commands to delete :c")
      rl.close();
    }
  );
})();
