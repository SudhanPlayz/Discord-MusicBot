...
async function runDelete(client, interaction, options) {
		...
		return interaction.reply({
			embeds: [
				new EmbedBuilder()
					.setColor(client.config.embedColor)
					.setDescription(
						"I couldn't find a playlist with that name"
					),
			],
			ephemeral: true,
		});
		...
		return interaction.reply({
			embeds: [
				new EmbedBuilder()
					.setColor(client.config.embedColor)
					.setDescription(
						"You can't delete a playlist that isn't yours"
					),
			],
			ephemeral: true,
		});
	...
	return interaction.reply({
		embeds: [
			new EmbedBuilder()
				.setColor(client.config.embedColor)
				.setDescription(`Deleted playlist **${playlist.name}**`),
		],
		ephemeral: true,
	});
}
	...
	.setAutocompleteOptions(async (input, index, interaction, client) => {
		if (client.db) {
			if (index == 1) {
				...
				if (operation === "remove") {
					const playlistName =
						interaction.options.getString("playlist_name");
	...
	.setRun(async (client, interaction, options) => {
		...
		if (operation === "view") {
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription(
								"You can't add songs to a playlist that isn't yours"
							),
					],
					ephemeral: true,
				});
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription(
								"You need to provide a song to add"
							),
					],
					ephemeral: true,
				});
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription(
								"I couldn't find a song with that name"
							),
					],
					ephemeral: true,
				});
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription(
								"You need to provide a song to remove"
							),
					],
					ephemeral: true,
				});
			...
			return interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setColor(client.config.embedColor)
						.setDescription(
							`Removed **${songData.name}** from **${playlist.name}**`
						),
				],
				ephemeral: true,
			});
				...
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription(
								"You can't play a playlist that isn't yours"
							),
					],
					ephemeral: true,
				});
			...
			if (!songs.length)
				return interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setColor(client.config.embedColor)
							.setDescription("That playlist is empty"),
					],
					ephemeral: true,
				});
			...
			for (const song of songs) {
				const songSearch = await player.search(
					song.link,
					interaction.user.id
				);
				player.players
					.get(interaction.guild.id)
					.queue.add(songSearch.tracks[0]);
			}
