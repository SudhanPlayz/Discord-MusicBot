...
class DiscordMusicBot extends Client {
	...
	build() {
		...
		this.manager = new Manager({
			...
			.on("queueEnd", /** @param {EpicPlayer} */ async (player, track) => {
	...
	createController(guild, player) {
		return new MessageActionRow().addComponents(
			...
			new MessageButton()
				...
				.setEmoji("⏮️")
				.setDisabled((!player.queue.previous || player.queue.previous !== player.queue.current || player.queue.previous !== player.queue[0]) ? true : false),
			...
			new MessageButton()
				.setStyle(player.trackRepeat ? "SUCCESS" : player.queueRepeat ? "SUCCESS" : "SECONDARY")
