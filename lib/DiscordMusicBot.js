...
import jsoning from "jsoning"; // Documentation: https://jsoning.js.org/
import { Manager } from "erela.js";
import ConfigFetcher from "../util/getConfig.js";
import Logger from "./Logger.js";
import spotify from "better-erela.js-spotify";
import { default as AppleMusic } from "better-erela.js-apple";
...
class DiscordMusicBot extends Client {
  ...
  constructor(
    ...
    this.log("Make sure to fill in the config.js before starting the bot.")
    ...
    fs.readdir(SlashCommandsDirectory, (err, files) => {
      if (err) throw err;
        ...
        files.forEach(async (file) => {
          ...
          this.slashCommands.set(file.split(".")[0].toLowerCase(), cmd.default);
    ...
    fs.readdir(ContextCommandsDirectory, (err, files) => {
      if (err) throw err;
        ...
        files.forEach(async (file) => {
          ...
          if (!cmd.default || !cmd.default.command || !cmd.default.run)
            return this.warn(
              "Unable to load Command: " +
                file.split(".")[0] +
                ", File doesn't have either command/run"
            );
          this.contextCommands.set(
